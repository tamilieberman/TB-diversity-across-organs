function build_candidate_mutation_table_tb(folder)
...
REF_GENOME_DIRECTORY = [masterdir '/MTB_anc/'];
other_positions_to_consider={[masterdir '/tools/lineage_defining_snps_from_Coll.mat']};
...
fprintf(['Finding mutations in samples specified in: ' folder  'sample_names.csv\n']);
...
cp = generate_positions(SampleDirs, SampleNames, looseFQmax, RUN_ON_COMPUTING_CLUSTER, jobsubmitoptions_short, TEMPORARYFOLDER,SCRIPTSDIRECTORY);
fprintf(1,['Found ' num2str(length(cp)) ' positions where provided vcf called a fixed variant in at least one sample with FQ score < ' num2str(looseFQmax) '\n']) ;


fprintf(1,'\nFinding single nucleotide positions with within-sample polymorphisms...\n');
[dp, coveragethresholds] = find_diverse_positions_no_control(loose_parameters, SampleDirs, SampleNames, RUN_ON_COMPUTING_CLUSTER, jobsubmitoptions_short,TEMPORARYFOLDER,SCRIPTSDIRECTORY);
